...
class CountingUI:
    ...
    def _create_widgets(self):
        ...
        line_frame = tk.Frame(model_frame, bg=self.C['card'])
        line_frame.pack(fill=tk.X)
        tk.Label(line_frame, text="计数线:", bg=self.C['card'], fg=self.C['text'],
                 font=("Arial", 9)).pack(side=tk.LEFT)
        tk.Label(line_frame, text="顶端", bg=self.C['card'], fg=self.C['text_muted'],
                 font=("Arial", 9)).pack(side=tk.LEFT)
        self.line_top_var = tk.StringVar(value=str(self.config.line_top_ratio))
        self.line_top_entry = ttk.Entry(line_frame, textvariable=self.line_top_var, width=5)
        self.line_top_entry.pack(side=tk.LEFT, padx=2)
        tk.Label(line_frame, text="%", bg=self.C['card'], fg=self.C['text_muted'],
                 font=("Arial", 9)).pack(side=tk.LEFT)
        tk.Label(line_frame, text="底端", bg=self.C['card'], fg=self.C['text_muted'],
                 font=("Arial", 9)).pack(side=tk.LEFT, padx=(10, 0))
        self.line_bot_var = tk.StringVar(value=str(self.config.line_bot_ratio))
        self.line_bot_entry = ttk.Entry(line_frame, textvariable=self.line_bot_var, width=5)
        self.line_bot_entry.pack(side=tk.LEFT, padx=2)
        tk.Label(line_frame, text="%", bg=self.C['card'], fg=self.C['text_muted'],
                 font=("Arial", 9)).pack(side=tk.LEFT)
        self.line_top_var.trace_add("write", self._on_line_top_change)
        self.line_bot_var.trace_add("write", self._on_line_bot_change)
    ...
    def _on_line_top_change(self, *args):
        """计数线顶端百分比变化回调"""
        try:
            v = int(self.line_top_var.get())
            self.config.line_top_ratio = max(0, min(100, v))
        except ValueError:
            pass

    def _on_line_bot_change(self, *args):
        """计数线底端百分比变化回调"""
        try:
            v = int(self.line_bot_var.get())
            self.config.line_bot_ratio = max(0, min(100, v))
        except ValueError:
            pass
...
def main():
    ...
    def line_x_at(cy):
        """给定y坐标, 返回斜线在该y处的x坐标（从cfg实时读取参数）"""
        lx1 = int(width * cfg.line_top_ratio / 100.0)
        lx2 = int(width * cfg.line_bot_ratio / 100.0)
        return lx1 + (lx2 - lx1) * cy / height
    ...
    while True:
        ...
        if frame is not None and frame.size > 0:
            lx1 = int(width * cfg.line_top_ratio / 100.0)
            lx2 = int(width * cfg.line_bot_ratio / 100.0)
            cv2.line(frame, (lx1, 0), (lx2, height), (0, 255, 0), 2)
